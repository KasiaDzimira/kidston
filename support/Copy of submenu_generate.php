		...
		<?PHP
			...
			<?PHP
				if($lo[$i]["status"] == "N")
				 {
		?>		
		<a style="width:<?PHP echo $wid1; ?>px;" href="<?PHP echo SITE_URL.$db->stringToUrlSlug($lo[$i][fol])."/".$lo[$i][cid]."-".$db->stringToUrlSlug($lo[$i][menu_name]); ?>"><font color="#FF0000"><?PHP echo $lo[$i]["menu_name"]; ?></font></a>
		<?PHP
				}
				else
				{
...
?>					
						...
						// This coding for change the status Inacvite color change... 
							...
							{
						?>
<a style="width:<?PHP echo $wid2; ?>px;" href="<?PHP echo SITE_URL.$db->stringToUrlSlug($lo[$i][fol])."/".$lt[$i][cid]."-".$db->stringToUrlSlug($lt[$i][menu_name]); ?>"><font color="#FF0000"><?PHP echo $lt[$j]["menu_name"]; ?></font></a>
						<?PHP 
						     } else { 
?>							 
<a style="width:<?PHP echo $wid2; ?>px;" href="<?PHP echo SITE_URL.$db->stringToUrlSlug($lo[$i][fol])."/".$lt[$i][cid]."-".$db->stringToUrlSlug($lt[$i][menu_name]); ?>"><?PHP echo $lt[$j]["menu_name"]; ?></a>
<?PHP
							 } 
						?>	
		<ul>
...
<?PHP					  
						...
						// This coding for change the status Inacvite color change... 
							...
							{
						?>
						<a style="width:<?PHP echo $wid3; ?>px;" href="<?PHP echo SITE_URL.$db->stringToUrlSlug($lo[$i][fol])."/".$lt[$i][cid]."-".$db->stringToUrlSlug($lth[$k][menu_name]); ?>"><font color="#FF0000"><?PHP echo $lth[$k]["menu_name"]; ?></font></a>
						<?PHP 
						     } else { 
						?>
						<a style="width:<?PHP echo $wid3; ?>px;" href="<?PHP echo SITE_URL.$db->stringToUrlSlug($lo[$i][fol])."/".$lt[$i][cid]."-".$db->stringToUrlSlug($lth[$k][menu_name]); ?>"><?PHP echo $lth[$k]["menu_name"]; ?></a>
						<?PHP
							 } 
						?>						
	</li>
