...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td height="50" align="left" valign="top"><b>Hired Candidates</b></td>
  </tr>
   <tr>
...
<?PHP echo html_entity_decode($rs_image[$i]["name"])." ".html_entity_decode($rs_image[$i]["lastname"]); ?></a><br />
					...
					<?PHP
							...
							$aplink = $db->fetchSingleRow("select al.remarks,al.application_status,DATE_FORMAT(al.auth_date,'%D %M, %Y') as dd,al.auth_by,aa.name from application_link al,admin_access aa where al.application_id='".$rs_image[$i]["appid"]."' and al.application_status='OFFERED' and al.auth_by = aa.ID");
					...
					<a href="admin_support/new_remarks.php?alid=<?PHP echo $rs_image[$i]["appid"]; ?>&rsrc=<?PHP echo $_REQUEST["src"]; ?>" title="Read Remarks" class="example6" id="watch">RR</a>
							...
							?>
					&nbsp;
				<a href="javascript:;" onClick="jform('admin_process/candidate_status.php?apid=<?PHP echo $rs_image[$i][appid]; ?>&st=OFFERED&src=<?PHP echo $_REQUEST[src]; ?>&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change');">AR</a>		
					&nbsp;
