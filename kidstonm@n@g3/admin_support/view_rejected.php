...
?>	</td>
  ...
  <tr>
    <td align="left">
			<span style="float:right; padding-right:15px;font-size:11px;"> AR -Add Remarks / RR - Read Remarks / DEL - Delete </span>
...
<?PHP echo html_entity_decode($rs_image[$i]["name"])." ".html_entity_decode($rs_image[$i]["lastname"]); ?></a><br />
					...
					<?PHP
						$aplink = $db->fetchSingleRow("select al.ID,al.application_status,al.remarks,DATE_FORMAT(al.auth_date,'%D %M, %Y') as dd,al.auth_by,aa.name from application_link al,admin_access aa where al.application_id='".$rs_image[$i]["appid"]."' and al.auth_by = aa.ID and al.application_status='REJECTED'");
				...
				$msg = "";
							...
							?>					
							&nbsp;
					<a href="javascript:;" onClick="jform('admin_process/candidate_status.php?apid=<?PHP echo $rs_image[$i][appid]; ?>&st=REJECTED&src=<?PHP echo $_REQUEST[src]; ?>&chk_st=Y&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change');">AR</a>	
					&nbsp;
