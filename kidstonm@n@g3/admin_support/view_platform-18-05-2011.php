<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td height="50" align="left" valign="top"><b>Platform Count</b></td>
  </tr>
 <?PHP
			$page = $_GET['page'];
			$limit = 75;
...
?>
				  <tr align="center" valign="middle" onmouseover="changeCSS(this,'#FBFBE7','#FCFAF3','#535353');this.style.cursor='pointer'" onmouseout="changeCSS(this,'#F9F9F9','#FCFAF3','#535353');">
					...
					<span class="smtxt">
					<?PHP echo $rs_image[$i]["platform_name"]; ?>			
					</span></td>
					<td align="left" bgcolor="#FFFFFF" style="padding-left: 10px;">
					<span class="smtxt">
					<?PHP
...
?>				
	<tr align="center" valign="middle">
			...
			<span class="smtxt">
			<?PHP
					$sql_cnth = $db->getTableArray("SELECT cm.ID AS CID,am.ID as aid,am.application_status,am.applied_date FROM application_master am, candidate_master cm WHERE cm.ID = am.candidate_id AND (am.application_status<>'DELETED') and cm.reference_name <> '' group by am.candidate_id");
					$cnt_sqlh = count($sql_cnth);
					$sql_cnta = $db->getTableArray("SELECT cm.ID AS CID,am.ID as aid,am.application_status,am.applied_date FROM application_master am, candidate_master cm WHERE cm.ID = am.candidate_id AND am.application_status =  'APPLIED' and cm.reference_name <> '' group by am.candidate_id");
			...
			<span class="smtxt">
			<?PHP
				$sql_cntg = $db->getTableArray("SELECT cm.ID AS CID,am.ID as aid,am.application_status,am.applied_date FROM application_master am, candidate_master cm,application_link alm WHERE cm.ID = am.candidate_id AND  alm.application_id=am.ID and alm.application_status='UNQUALIFIED' and cm.reference_name <> '' group by am.candidate_id");
