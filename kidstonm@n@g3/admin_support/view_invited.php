...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td height="50" align="left" valign="top"><b>Interviewed Candidates</b></td>
  </tr>
   <tr>
...
<?PHP
			...
			{// jcode
				...
				$sql_image_pg = "select cd.ID,cd.name,cd.lastname,cd.sex,cd.dob,cd.address,cd.street,cd.city,cd.state,cd.country,cd.postalcode,cd.contact_number,cd.email,cd.applied_date as candidate_apply_date,cd.reference,cd.reference_name,cd.status,jm.ID as jid,jm.org_id,jm.job_code, jm.job_title, jm.job_asap, jm.job_brief,jm.location,DATE_FORMAT(jm.apply_date,'%D %M, %Y') as dd,jm.admin_id,jm.status as jstatus,ap.ID as appid,ap.resume,ap.resume2,ap.resume3,ap.applied_date as application_apply_date,ap.application_status from candidate_master cd,job_details jm,application_master ap,company_details cm where jm.job_code ='$get_jobcode' and cd.status = 'Y' and cm.status ='Y' and ap.application_status = 'INVITED' and cd.ID = ap.candidate_id and jm.org_id = cm.ID and jm.ID = ap.job_id order by ap.ID desc";
			}// jcode
			else
			{// jcode
				$sql_image_pg = "select cd.ID,cd.name,cd.lastname,cd.sex,cd.dob,cd.address,cd.street,cd.city,cd.state,cd.country,cd.postalcode,cd.contact_number,cd.email,cd.applied_date as candidate_apply_date,cd.status,cd.reference,cd.reference_name,jm.ID as jid,jm.org_id, jm.job_code, jm.job_title, jm.job_asap, jm.job_brief,jm.location,DATE_FORMAT(jm.apply_date,'%D %M, %Y') as dd,jm.admin_id,jm.status as jstatus,ap.ID as appid,ap.resume,ap.resume2,ap.resume3,ap.applied_date as application_apply_date,ap.application_status from candidate_master cd,job_details jm,application_master ap,company_details cm where cd.status = 'Y' and cm.status ='Y' and ap.application_status = 'INVITED' and cd.ID = ap.candidate_id and jm.org_id = cm.ID and jm.ID = ap.job_id order by ap.ID desc";
			}// jcode
		//echo $sql_image_pg;
...
?>	</td>
  ...
  <tr>
    <td align="left">
				<span style="float:right; padding-right:15px;font-size:11px;">PR - Present / REJ - Reject / DEL - Delete / RR - Read Remarks / ER - Edit Remarks</span>
...
<?PHP echo html_entity_decode($rs_image[$i]["name"])." ".html_entity_decode($rs_image[$i]["lastname"]); ?></a><br />
					...
					<?PHP
							...
							{// status
					?>
					<a href="javascript:;" onClick="jform('admin_process/candidate_status.php?apid=<?PHP echo $rs_image[$i][appid]; ?>&st=PRESENTED&src=<?PHP echo $_REQUEST[src]; ?>&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change');">PR</a>
					&nbsp;
					...
					<!--<a href="admin_process/candidate_delstatus.php?apid=<?PHP echo $rs_image[$i][appid]; ?>&st=DELETED&src=<?PHP echo $_REQUEST[src]; ?>&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change">DEL</a>-->			
					...
					<a href="javascript:;" onClick="cfrm('<?PHP echo $msg1; ?>','admin_process/candidate_delstatus.php?cid=<?PHP echo $rs_image[$i][ID]; ?>&apid=<?PHP echo $rs_image[$i][appid]; ?>&st=DELETED&src=<?PHP echo $_REQUEST[src]; ?>&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change');">DEL</a>	
						<?PHP
							...
							$aplink = $db->fetchSingleRow("select al.ID,al.remarks,DATE_FORMAT(al.auth_date,'%D %M, %Y') as dd,al.auth_by,aa.name from application_link al,admin_access aa where al.application_id='".$rs_image[$i]["appid"]."' and al.application_status='INVITED' and al.auth_by = aa.ID");
					...
					?>
					&nbsp;
				<a href="javascript:;" onClick="jform('admin_process/candidate_status.php?apid=<?PHP echo $rs_image[$i][appid]; ?>&st=INVITED&src=<?PHP echo $_REQUEST[src]; ?>&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change');">AR</a>
					</td>
				  </tr>
<?PHP
				$sn = $sn + 1;
