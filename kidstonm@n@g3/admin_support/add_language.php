...
if($_REQUEST["act"] == "edit")
	{//if1
		$id = $_REQUEST["id"];
		//echo $id;
		$language_type= $db->fetchSingleRow("select * from language_master where ID = ".$id );
		$language_name = $language_type["language_name"];
		$language_status = $language_type["status"];
	}	
?>	
<form name="frm_language" id="frm_language" action="admin_process/language_save.php" method="post" onSubmit="return validation_language();">
      ...
      <td><table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td colspan="4">&nbsp;</td>
		        ...
		        <tr>
          <td><strong>Language Details </strong> </td>
         <td>&nbsp;</td>
		 ...
		 <?php if($_REQUEST["act"] == "edit")
			 {
		 ?>
         <a href="home.php?src=add-language">Create New</a>
		<?php
			}
		?>
        </td>
        ...
        <tr>
		  ...
		  <tr>
          <td width="228" height="35" valign="middle"><span class="bl-text">Language Name*</span></td>
          <td width="13" align="center" valign="middle"><strong>:</strong></td>
          <td colspan="4" align="left"><input name="language_title" type="text" class="field-job" id="language_title" value="<?php echo $language_name; ?>" /></td>
        </tr>
   <tr>
          <td height="35">&nbsp;</td>
          <td valign="middle">&nbsp;</td>
...
<?PHP
			...
			$sql_list = "select * from language_master order by language_name";
...
?>
    ...
    <tr class="title">
     ...
     <td align="center" valign="middle" bgcolor="#FFFFFF" colspan="2"><strong> Language </strong></td>
...
?>
    <tr>
	  ...
	  <?PHP echo $rs_list[$i]["language_name"];  ?></td>
	  ...
	  <?PHP 
			...
			{ 
	  ?>
	  		  <a href="admin_process/language_save.php?id=<?PHP echo $rs_list[$i]['ID']; ?>&st=N&act=sts&src=<?PHP echo $_REQUEST[src]; ?>&page=<?php echo $_REQUEST[page]; ?>" class="red-link">IN</a>
		<?PHP 		
			}
			else
			{ 
		?>
			  <a href="admin_process/language_save.php?id=<?PHP echo $rs_list[$i]['ID']; ?>&st=Y&act=sts&src=<?PHP echo $_REQUEST[src]; ?>&page=<?php echo $_REQUEST[page]; ?>" class="red-link">A</a>
		<?PHP 	
			} // else status
			?>&nbsp;
	  <a href="home.php?src=<?PHP echo $_REQUEST[src]; ?>&act=edit&id=<?PHP echo $rs_list[$i]['ID']; ?>&page=<?php echo $_REQUEST[page]; ?>" class="red-link">E</a> &nbsp;
          ...
          <a href="javascript:;" class="red-link" onclick="return cfrm('Are you sure want to  delete?','admin_process/language_save.php?act=del&id=<?PHP echo $rs_list[$i][ID]; ?>&page=<?php echo $_REQUEST[page]; ?>&src=<?PHP echo $_REQUEST['src']; ?>');">D</a>      
	  </td>
    </tr>
    <?PHP
		$sn +=  1;
