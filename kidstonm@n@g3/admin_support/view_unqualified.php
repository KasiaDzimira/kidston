...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td height="50" align="left" valign="top"><b>Rejected Candidates</b></td>
  </tr>
   <tr>
...
<?PHP
			...
			{// jcode
				...
				$sql_image_pg = "select cd.ID,cd.name,cd.lastname,cd.sex,cd.dob,cd.address,cd.street,cd.city,cd.state,cd.country,cd.postalcode,cd.contact_number,cd.email,cd.applied_date as candidate_apply_date,cd.status,cd.reference,cd.reference_name,jm.ID as jid,jm.org_id,jm.job_code, jm.job_title,jm.job_asap, jm.job_brief, jm.location,DATE_FORMAT(jm.apply_date,'%D %M, %Y') as dd,jm.admin_id,jm.status as jstatus,ap.ID as appid,ap.resume,ap.resume2,ap.resume3,ap.applied_date as application_apply_date,ap.application_status from candidate_master cd,job_details jm,application_master ap,company_details cm where jm.job_code ='$get_jobcode' and cd.status = 'Y' and ap.application_status = 'UNQUALIFIED' and cd.ID = ap.candidate_id and jm.org_id = cm.ID and jm.ID = ap.job_id order by ap.ID desc";
			}// jcode
			else
			{// jcode
				$sql_image_pg = "select cd.ID,cd.name,cd.lastname,cd.sex,cd.dob,cd.address,cd.street,cd.city,cd.state,cd.country,cd.postalcode,cd.contact_number,cd.email,cd.applied_date as candidate_apply_date,cd.status,cd.reference,cd.reference_name,jm.ID as jid,jm.org_id, jm.job_asap,jm.job_code, jm.job_title, jm.job_brief,jm.location,DATE_FORMAT(jm.apply_date,'%D %M, %Y') as dd,jm.admin_id,jm.status as jstatus,ap.ID as appid,ap.resume,ap.resume2,ap.resume3,ap.applied_date as application_apply_date,ap.application_status from candidate_master cd,job_details jm,application_master ap,company_details cm where cd.status = 'Y' and ap.application_status = 'UNQUALIFIED' and cd.ID = ap.candidate_id and jm.org_id = cm.ID and jm.ID = ap.job_id order by ap.ID desc";
			}// jcode
			//echo $sql_image_pg;
...
<?PHP echo html_entity_decode($rs_image[$i]["name"])." ".html_entity_decode($rs_image[$i]["lastname"]); ?></a><br />
					...
					<?PHP
												$aplink = $db->fetchSingleRow("select al.ID,al.application_status,al.remarks,DATE_FORMAT(al.auth_date,'%D %M, %Y') as dd,al.auth_by,aa.name from application_link al,admin_access aa where al.application_id='".$rs_image[$i]["appid"]."' and al.auth_by = aa.ID and al.application_status='UNQUALIFIED'");
				...
				$msg = "";
						...
						?>			
							&nbsp;
					<a href="javascript:;" onClick="jform('admin_process/candidate_status.php?apid=<?PHP echo $rs_image[$i][appid]; ?>&st=UNQUALIFIED&src=<?PHP echo $_REQUEST[src]; ?>&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change');">AR</a>			
								&nbsp;
