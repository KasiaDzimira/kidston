...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td height="50" align="left" valign="top"><b>Presented Candidates</b></td>
  </tr>
   <tr>
...
?>	</td>
  ...
  <tr>
    <td align="left">
	<span style="float:right; padding-right:15px;font-size:11px;">H - Hired(Offered) / REJ - Reject / DEL - Delete / RR - Read Remarks / ER - Edit Remarks</span>
...
<?PHP echo html_entity_decode($rs_image[$i]["name"])." ".html_entity_decode($rs_image[$i]["lastname"]); ?></a><br />
					...
					<?PHP
							...
							{// status
					?>
					<a href="javascript:;" onClick="jform('admin_process/candidate_status.php?apid=<?PHP echo $rs_image[$i][appid]; ?>&st=OFFERED&src=<?PHP echo $_REQUEST[src]; ?>&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change');">H</a>&nbsp;
				...
				<?PHP
							...
							$aplink = $db->fetchSingleRow("select al.ID,al.remarks,DATE_FORMAT(al.auth_date,'%D %M, %Y') as dd,al.auth_by,aa.name from application_link al,admin_access aa where al.application_id='".$rs_image[$i]["appid"]."' and al.application_status='PRESENTED' and al.auth_by = aa.ID");
				...
				$msg = "";
					...
					?>
					&nbsp;
				<a href="javascript:;" onClick="jform('admin_process/candidate_status.php?apid=<?PHP echo $rs_image[$i][appid]; ?>&st=PRESENTED&src=<?PHP echo $_REQUEST[src]; ?>&page=<?PHP echo $_REQUEST[page]; ?>&frm_jobcode=<?PHP echo $_REQUEST[frm_jobcode]; ?>&btsave=change');">AR</a>
					</td>
				  </tr>
<?PHP
				$sn = $sn + 1;
